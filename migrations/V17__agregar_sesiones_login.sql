-- ============================================================
-- SESIONES Y TOKENS DE RECUPERACION PARA ACADEMICO-LOGIN (V17)
-- ============================================================
--
-- Centraliza en academico-esquema-bd las tablas que academico-login
-- consulta para persistencia de sesiones, revocacion de refresh tokens
...
COMMENT ON TABLE academico.password_reset_tokens IS
  'Tokens de recuperacion de contrasena emitidos por academico-login. Solo se almacena hash SHA-256 del token.';

DO $$
BEGIN
  IF to_regclass('academico.auth_sessions') IS NULL THEN
    RAISE EXCEPTION 'No se pudo crear academico.auth_sessions.';
  END IF;

  IF to_regclass('academico.password_reset_tokens') IS NULL THEN
    RAISE EXCEPTION 'No se pudo crear academico.password_reset_tokens.';
  END IF;
END $$;
